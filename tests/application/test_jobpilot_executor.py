...
def test_jobpilot_executor_builds_campaign_and_dispatches_browser(monkeypatch):
    ...
    executor = JobPilotExecutor(
        api_url="http://jobpilot/api",
        api_token="test-token",
        terminal_url="http://terminal",
        timeout=0,
        poll_interval=0,
    )
    ...
    result = executor.execute(
        {"application_url": "https://example.com/apply", "title": "Engineer", "company": "Example"},
        {},
        "/tmp/resume.pdf",
    )
    ...
    assert any(path == "/api/campaigns" and body["source"] == "career-os" for kind, _, path, body in calls if kind == "api")
...
def test_jobpilot_executor_refuses_duplicate_before_browser(monkeypatch):
    ...
    result = executor.execute({"application_url": "https://example.com/apply", "title": "Engineer", "company": "Example"}, {}, "resume.pdf")
