...
def _chunks(value: Any, size: int = 1900) -> list[str]:
    text = str(value or "")
...
def value_for(prop_type: str, value: Any) -> dict[str, Any] | None:
    ...
    if prop_type == "rich_text":
        return {"rich_text": [{"type": "text", "text": {"content": chunk}} for chunk in _chunks(value)]}
    if prop_type == "url":
...
def existing_keys(token: str, data_source_id: str, props: dict[str, dict[str, Any]]) -> set[str]:
    ...
    while True:
        ...
        response = notion_request(f"/data_sources/{data_source_id.replace('-', '')}/query", token, method="POST", body=body)
