...
## ARACHNE live control plane and web application

The ARACHNE product UI replaces `dashboard/index.html` as a static mock with a live web application connected to real persisted Career OS state - no hardcoded metrics.

- **Control-plane API** (`src/career_os/arachne_control.py`): one router behind the UI exposing overview, jobs/job detail with full analytic packages, approval queue, approve/withdraw (human gate only), executions, history, activity, providers, candidate, base resume, interview, learning, and a web graph. Reads are real; the only mutations are human approve/withdraw.
- **Persist full analytic artifacts**: `EndToEndOrchestrator.prepare` now stores JD analysis, evidence ledger, tailored resume bullets + matched keywords, ATS audit, and recruiter review in the execution `pipeline` dict, so the frontend renders first-class JD, evidence-backed fit, visual resume review, ATS audit and recruiter review per job.
- **Frontend** (`dashboard/index.html`): single-page ARACHNE app with a spider-web visual identity and purposeful motion. Views: Overview, Web (real entity graph as radial web), Job Discovery, Job Detail (JD/fit/evidence/resume/ATS/recruiter + approval actions), Approval Queue, Execution (live `READY→…→VERIFIED` with explicit failed/blocked states, polling), Workflow History, Agent Activity, Interview Prep, Learning, Providers.
- **Tests**: new `tests/test_arachne_control.py` (10 tests) covering overview, discover-demo, full job detail pipeline, approval transitions + idempotency guard, withdraw, 404s, providers/candidate/base-resume, interview, learning, and graph. Full local suite: **359 passed**, 3 pre-existing environment-only PDF failures (Playwright/Chromium executable unavailable; `playwright` package not installable in this sandbox's package index). Frontend validated by node JS syntax check, element/handler reference check, and end-to-end API smoke test through the packaged app.
